...
case class TimerCal(dow: List[DayOfWeek],
  ...
  def nextTimers(now: LocalDateTime): Stream[LocalDateTime] = {
    val startYear = now.getYear
    def nextMonths(y: Int) =
      if (month.isEmpty) {
        if (y == now.getYear) (now.getMonthValue to 12)
        else (1 to 12)
      } else {
        month
      }
    def nextDays(y: Int, m: Int) =
      if (day.isEmpty) {
        if (y == now.getYear && m == now.getMonthValue) (now.getDayOfMonth to 31)
        else (1 to 31)
      } else {
        day
      }
    def nextHour(y: Int, m: Int, d: Int) =
      if (hour.isEmpty) {
        if (y == now.getYear && m == now.getMonthValue && d == now.getDayOfMonth) (now.getHour to 59)
        else (0 to 59)
      } else {
        hour
      }
  ...
  def nextTrigger(ref: LocalDateTime): Option[LocalDateTime] =
    nextTimers(ref).find(_ isAfter ref)

  def nextTriggerDelay(ref: LocalDateTime): Option[FiniteDuration] =
    nextTrigger(ref).
      map(ld => Duration.between(LocalDateTime.now, ld)).
